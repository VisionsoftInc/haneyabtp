          ...
          debugger
            ...
            if (iSelected === 0) sStatus = "SUCCESS";
             ...
             var aFilters = [
                new Filter({
                    filters: [
                        new Filter("CREATED_ON", FilterOperator.GE, sStart),
                        new Filter("CREATED_ON", FilterOperator.LE, sEnd)
                    ],
                    and: true
                }),
                new Filter("REMARKS", FilterOperator.EQ,  sStatus)
            ];

             this.getOwnerComponent()
             .getModel("UiLoadingStatus")
                     .setProperty("/busy", true);
              this.oModel.read("/EVT_CONSIGNSet", {
                filters: aFilters,
                success: function (oData) {
                    console.log("Success:", oData.results);
                       this.oUiModel.setProperty("/busy", false);
                    // Example: bind to JSON model
                    var oConsignmentModel = this.getOwnerComponent().getModel("ConsignmentModel");
                      oConsignmentModel.setData({
         records: oData.results
    });
                    this.getOwnerComponent().getRouter().navTo("AiRPAConsignmentSalesOutput");
                }.bind(this),
